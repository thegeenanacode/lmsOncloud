...
type RestOf<T extends IProject | NewProject> = Omit<T, 'submissionDate' | 'createdDate' | 'lastModifiedDate'> & {
...
export class ProjectService {
  ...
  protected convertDateFromClient<T extends IProject | NewProject | PartialUpdateProject>(project: T): RestOf<T> {
    return {
      ...project,
      submissionDate: project.submissionDate?.format(DATE_FORMAT) ?? null,
  ...
  protected convertDateFromServer(restProject: RestProject): IProject {
    return {
      ...restProject,
      submissionDate: restProject.submissionDate ? dayjs(restProject.submissionDate) : undefined,
