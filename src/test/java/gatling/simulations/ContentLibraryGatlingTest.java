...
/**
 * Performance test for the ContentLibrary entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.6.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class ContentLibraryGatlingTest extends Simulation {

    String baseURL = Optional.ofNullable(System.getProperty("baseURL")).orElse("http://localhost:7575");
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all contentLibraries").get("/api/content-libraries").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new contentLibrary")
                        .post("/api/content-libraries")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_contentLibrary_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created contentLibrary").get("${new_contentLibrary_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created contentLibrary").delete("${new_contentLibrary_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ContentLibrary entity").exec(scn);
